...
<c:forEach items="${it.data.list}" var="model">
    <li><p><span>${model.bktmc}</span></p><b class="ui-icon-plus"></b></li>
    <div class="panel" style="display:none;">
        <p>报刊亭名称：${model.bktmc}</p>
        <p>报刊亭地址：${model.bktdz}</p>
        <p>坐标：${model.zb}</p>
    </div>    
</c:forEach>
...
$(function(){
	$(".get_more").click(function() {
		...
		$.ajax({
		    ...
		    dataType : 'json',
			type : 'GET',
			url : basePath + "rest/transport/bktxx/getMore?start="+length,
			success : function(res) {
			    var data = res.data;
				if (data && data.list) {
					$.each(data.list, function(_,m) {
						var html = "<li><p><span>"+m.bktmc+"</span></p><b class='ui-icon-plus'></b></li>"
					        + "<div class='panel' style='display:none;'>"
				            + "<p>报刊亭名称："+m.bktmc+"</p>"
				            + "<p>报刊亭地址："+m.bktdz+"</p>"
				            + "<p>坐标："+m.zb+"</p></div>";

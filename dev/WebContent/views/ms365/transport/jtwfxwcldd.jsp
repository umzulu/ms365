...
<c:forEach items="${it.data.list}" var="model">
    <li><p><span>${model.cld}</span></p><b class="ui-icon-plus"></b></li>
...
$(function(){
	$(".get_more").click(function() {
		...
		$.ajax({
		    ...
		    dataType : 'json',
			type : 'GET',
			url : basePath + "rest/transport/jtwfxwcldd/getMore?start="+length,
			success : function(res) {
			    var data = res.data;
				if (data && data.list) {
					$.each(data.list, function(_,m) {
						var html = "<li><p><span>"+m.cld+"</span></p><b class='ui-icon-plus'></b></li>"
					        + "<div class='panel' style='display:none;'>"
				            + "<p>处理点："+m.cld+"</p>"
				            + "<p>地址："+m.dz+"</p>"
				            +"<p>联系电话"+m.lxdh+"</p></div>";

...
<c:forEach items="${it.data.list}" var="model">
    <li><p><span>${model.bh}</span></p><b class="ui-icon-plus"></b></li>
...
$(function(){
	$(".get_more").click(function() {
		...
		$.ajax({
		    ...
		    dataType : 'json',
			type : 'GET',
			url : basePath + "rest/transport/ggtcc/getMore?start="+length,
			success : function(res) {
			    var data = res.data;
				if (data && data.list) {
					$.each(data.list, function(_,m) {
						var html = "<li><p><span>"+m.bh+"</span></p><b class='ui-icon-plus'></b></li>"
					        + "<div class='panel' style='display:none;'>"
				            + "<p>编号："+m.bh+"</p>"
				            + "<p>名称："+m.mc+"</p>"
				            + "<p>地点："+m.dd+"</p>"

...
<c:forEach items="${it.data.list}" var="model">
    <li><p><span>${model.lb}</span></p><b class="ui-icon-plus"></b></li>
...
$(function(){
	$(".get_more").click(function() {
		...
		$.ajax({
		    ...
		    dataType : 'json',
			type : 'GET',
			url : basePath + "rest/transport/jzxl/getMore?start="+length,
			success : function(res) {
			    var data = res.data;
				if (data && data.list) {
					$.each(data.list, function(_,m) {
						var html = "<li><p><span>"+m.lb+"</span></p><b class='ui-icon-plus'></b></li>"
					        + "<div class='panel' style='display:none;'>"
				            + "<p>类别："+m.lb+"</p>"
				            + "<p>航线："+m.hx+"</p>"
				            + "<p>班轮周期："+m.blzq+"</p>"

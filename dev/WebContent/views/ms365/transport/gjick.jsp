...
<c:forEach items="${it.data.list}" var="model">
    <li><p><span>${model.czd}</span></p><b class="ui-icon-plus"></b></li>
...
$(function(){
	$(".get_more").click(function() {
		...
		$.ajax({
		    ...
		    dataType : 'json',
			type : 'GET',
			url : basePath + "rest/transport/gjick/getMore?start="+length,
			success : function(res) {
			    var data = res.data;
				if (data && data.list) {
					$.each(data.list, function(_,m) {
						var html = "<li><p><span>"+m.czd+"</span></p><b class='ui-icon-plus'></b></li>"
					        + "<div class='panel' style='display:none;'>"
				            + "<p>充值点："+m.czd+"</p>"
				            + "<p>地址："+m.dz+"</p>"
				            + "<p>办理内容："+m.blnr+"</p>"

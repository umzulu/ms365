...
<c:forEach items="${it.data.list}" var="model">
    <li><p><span>${model.fwzmc}</span></p><b class="ui-icon-plus"></b></li>
    <div class="panel" style="display:none;">
        <p>服务站名称：${model.fwzmc}</p>
        <p>服务处：${model.fwc}</p>
        <p>电话：${model.dh}</p>
    </div>    
</c:forEach>
...
$(function(){
	$(".get_more").click(function() {
		...
		$.ajax({
		    ...
		    dataType : 'json',
			type : 'GET',
			url : basePath + "rest/transport/tlbsccx/getMore?start="+length,
			success : function(res) {
			    var data = res.data;
				if (data && data.list) {
					$.each(data.list, function(_,m) {
						var html = "<li><p><span>"+m.fwzmc+"</span></p><b class='ui-icon-plus'></b></li>"
					        + "<div class='panel' style='display:none;'>"
				            + "<p>服务站名称："+m.fwzmc+"</p>"
				            + "<p>服务处："+m.fwc+"</p>"
				            +"<p>电话"+m.dh+"</p></div>";

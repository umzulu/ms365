...
<c:forEach items="${it.data.list}" var="model">
    <li><p><span>${model.jsmc}</span></p><b class="ui-icon-plus"></b></li>
    <div class="panel" style="display:none;">
        <p>局所名称：${model.jsmc}</p>
        <p>地区：${model.dq}</p>
        <p>邮编：${model.yb}</p>
        <p>地址：${model.dz}</p>
        <p>日营业时长：${model.ryysc}</p>
        <p>联系电话：${model.lxdh}</p>
    </div>    
</c:forEach>
...
$(function(){
	$(".get_more").click(function() {
		...
		$.ajax({
		    ...
		    dataType : 'json',
			type : 'GET',
			url : basePath + "rest/housing/yzjyyt/getMore?start="+length,
			success : function(res) {
			    var data = res.data;
				if (data && data.list) {
					$.each(data.list, function(_,m) {
						var html = "<li><p><span>"+m.jsmc+"</span></p><b class='ui-icon-plus'></b></li>"
					        + "<div class='panel' style='display:none;'>"
				            + "<p>局所名称："+m.jsmc+"</p>"
				            + "<p>地区："+m.dq+"</p>"
				            + "<p>邮编："+m.yb+"</p>"

...
<c:forEach items="${it.data.list}" var="model">
    <li><p><span>${model.lxsmc}</span></p><b class="ui-icon-plus"></b></li>
    <div class="panel" style="display:none;">
        <p>旅行社名称：${model.lxsmc}</p>
        <p>级别：${model.aj}</p>
        <p>地址：${model.dz}</p>
        <p>电话：${model.dh}</p>
    </div>    
...
$(function(){
	$(".get_more").click(function() {
		...
		$.ajax({
		    ...
		    dataType : 'json',
			type : 'GET',
			url : basePath + "rest/housing/lxs/getMore?start="+length,
			success : function(res) {
			    var data = res.data;
				if (data && data.list) {
					$.each(data.list, function(_,m) {
						var html = "<li><p><span>"+m.lxsmc+"</span></p><b class='ui-icon-plus'></b></li>"
					        + "<div class='panel' style='display:none;'>"
				            + "<p>旅行社名称："+m.lxsmc+"</p>"
				            + "<p>级别："+m.aj+"</p>"
				            + "<p>地址："+m.dz+"</p>"

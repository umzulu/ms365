...
<c:forEach items="${it.data.list}" var="model">
    <li><p><span>${model.tjjg}</span></p><b class="ui-icon-plus"></b></li>
...
$(function(){
	$(".get_more").click(function() {
		...
		$.ajax({
		    ...
		    dataType : 'json',
			type : 'GET',
			url : basePath + "rest/hospital/tjjg/getMore?start="+length,
			success : function(res) {
			    var data = res.data;
				if (data && data.list) {
					$.each(data.list, function(_,m) {
						var html = "<li><p><span>"+m.tjjg+"</span></p><b class='ui-icon-plus'></b></li>"
					        + "<div class='panel' style='display:none;'>"
					        + "<p>体检机构："+m.tjjg+"</p>"
					        + "<p>地址："+m.dz+"</p>"
				            + "<p>电话："+m.dh+"</p></div>";
